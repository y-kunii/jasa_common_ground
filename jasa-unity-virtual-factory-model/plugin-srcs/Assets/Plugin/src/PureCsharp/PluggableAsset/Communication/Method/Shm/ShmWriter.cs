...
namespace Hakoniwa.PluggableAsset.Communication.Method.Shm
{
    class ShmWriter : IIoWriter, IDisposable
    {
        ...
        public void Flush(IPduCommData data)
        {
            ...
            // SHM書き込み時に内容をログ出力
            UnityEngine.Debug.Log($"[SHM WRITE] asset_name={this.asset_name}, shm_asset_name={this.shm_config.asset_name}, channel_id={shm_config.channel_id}, size={buf.Length}, first_bytes={BitConverter.ToString(buf, 0, Math.Min(16, buf.Length))}");
